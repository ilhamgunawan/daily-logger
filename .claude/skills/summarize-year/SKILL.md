---
name: summarize-year
description: Generate an annual summary (logs/<year>/summary_<year>.md) from a year's finalized daily entries (and that year's monthly/weekly summaries where useful), following this repo's summary rules. Use when the user says "summarize the year", "generate annual summary", "annual summary for <year>", or names a year.
---

# Summarize Year

Produce one annual summary from a year's finalized daily logs. This skill
implements the annual part of the "Summarize" workflow described in
`AGENTS.md` and `README.md`.

## Inputs

Determine the target year from the user's request (`YYYY`, or "this
year"/"last year"). If ambiguous, ask which year to summarize rather than
guessing.
...
2. **Check the year has ended.** An annual summary should reflect a
   ...
   year, stop and confirm with the user before proceeding (they may
   explicitly want a partial-year summary).
...
3. **Collect the year's final entries.** Look for every
   ...
   **only** final entries — never drafts. If **no** final entries exist for
   the year, stop and tell the user instead of producing an empty summary.
