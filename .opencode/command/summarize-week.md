---
description: Generate a weekly summary from a week's finalized daily entries, following this repo's summary rules.
agent: build
---
Produce one ISO-week summary from that week's finalized daily logs. This
implements the weekly part of the "Summarize" workflow described in
`AGENTS.md` and `README.md`.

Target week: $ARGUMENTS
(An explicit `YYYY-Www`, a date that falls in the week, or "this
week"/"last week". If ambiguous, ask which week to summarize rather than
guessing.)
...
3. **Check the week has ended.** A weekly summary must not be created until
   ...
   the week is still in progress, stop and report that.
...
4. **Collect the week's final entries.** For each day Monday–Sunday, look
   ...
   stop and report that instead of producing an empty summary.
